        ...
        <div className="flex flex-col gap-3">
          {item.dynamic ? (
            <div className="flex items-center gap-2 group/label">
              ...
              <button 
                ...
                className={cn("p-1 transition-colors shrink-0", savedFields[customLabelKey] ? "text-green-600" : "text-neutral-400 group-hover/label:text-black")}
              >
                {savingFields[customLabelKey] ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className={cn("h-4 w-4", !savedFields[customLabelKey] && "opacity-20")} />}
              </button>
            </div>
          ) : (
            <Label className="text-[13px] font-normal text-black leading-tight flex-1">{item.label}</Label>
          )}
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-1 shrink-0">
              ...
              >
                <SelectTrigger className="h-8 w-24 border-none bg-transparent shadow-none p-0 text-[11px] uppercase font-normal text-black focus:ring-0">
            ...
            <div className="flex items-center gap-3 flex-1">
              <div className="relative flex-1">
                ...
                <button 
                  ...
                  className={cn("absolute right-0 top-1/2 -translate-y-1/2 p-1", savedFields[`${labelToUse}:obs`] ? "text-green-600" : "text-neutral-400 hover:text-black")}
                >
                  {savingFields[`${labelToUse}:obs`] ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5" />}
                </button>
              </div>
              <button 
                onClick={() => togglePhotos(item.id)}
                className={cn(
                  "h-8 w-8 flex items-center justify-center border transition-all shrink-0 rounded-sm",
                  openPhotos[item.id] ? "bg-black text-white border-black" : "bg-neutral-50 text-neutral-400 border-neutral-200 hover:border-black hover:text-black"
                )}
              >
          ...
          {openPhotos[item.id] && (
            <div className="animate-in slide-in-from-top-2 duration-300 border-t border-neutral-100 mt-1 pt-3">
              <PhotoRegistry 
                reportId={reportId} 
                formId={formId} 
                stationId={stationId} 
      ...
      <div className="space-y-0">
        {SECTIONS.map((section) => (
          <div key={section.title}>
            <div className="bg-black text-white px-4 py-2 flex items-center gap-2 sticky top-0 z-10">
